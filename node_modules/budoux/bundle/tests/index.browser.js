...
(() => {
      ...
      return Array.from(function* (blocks2) {
        for (const block of blocks2) {
          yield {
            ...
            after: block.nodes.map((block2) => block2.hasBreakOpportunityAfter)
          };
        }
      }(blocks));
    }
  ...
  describe("HTMLProcessingParser.applyToElement", () => {
    const checkEqual = (model2, inputHTML, expectedHTML) => {
      const inputDOM = parseFromString(inputHTML);
      const inputDocument = inputDOM.querySelector("p");
      const parser3 = new HTMLProcessingParser(model2);
      parser3.applyToElement(inputDocument);
    ...
    it("should insert ZWSPs where the sentence should break.", () => {
      ...
      checkEqual(model2, inputHTML, expectedHTML);
    });
  ...
  describe("HTMLProcessingParser.translateHTMLString", () => {
    ...
    const checkEqual = (model2, inputHTML, expectedHTML) => {
      const parser3 = new HTMLProcessingParser(model2);
      const result = parser3.translateHTMLString(inputHTML);
    ...
    it("should work with emojis.", () => {
      const inputHTML = "xyza\u{1F1EF}\u{1F1F5}\u{1F1F5}\u{1F1F9}abc";
      const expectedHTML = `<span
    style="word-break: keep-all; overflow-wrap: anywhere;"
    >xyz\u200Ba\u{1F1EF}\u{1F1F5}\u{1F1F5}\u{1F1F9}\u200Babc</span>`;
      checkEqual(defaultModel, inputHTML, expectedHTML);
    });
  });

  // module/tests/test_parser.js
  describe("Parser.parse", () => {
    ...
    it("should separate if a strong feature item supports.", () => {
      ...
      const parser3 = new Parser(model2);
      const result = parser3.parse(TEST_SENTENCE);
    ...
    it("should return an empty list when the input is a blank string.", () => {
      ...
      const result = parser3.parse("");
      expect(result).toEqual([]);
    });
  });

  // module/webcomponents/budoux-base.js
  ...
  var BudouXBaseElement = class extends HTMLElement {
    /**
  ...
  // module/webcomponents/budoux-ja.js
  var BudouXJapaneseElement = class extends BudouXBaseElement {
    /**
  ...
  // module/tests/test_webcomponents.js
  var parser2 = loadDefaultJapaneseParser();
  describe("Web Components", () => {
    ...
    it("should process the provided text.", () => {
      ...
      parser2.applyToElement(mirroredElement);
